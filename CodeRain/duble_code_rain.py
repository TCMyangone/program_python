...
texts = [font.render(str(i), True, (255, 0, 0)) for i in range(2)]
texts_1 = [font.render(str(i), True, (0, 0, 255)) for i in range(2)]
#计算有多少列代码雨
colums = int(width / font_ps)
#根据列数创建一个列表，元素初始值都为0
drops = [0 for i in range(colums)]
drops_1 = [int(height / font_ps)for i in range(colums)]
...
while True:
    ...
    for i in range(len(drops)):
        ...
        if drops[i] * font_ps > height * 2 or random.random() > 0.93:
            drops[i] = 0
    #倒着来
    for i in range(len(drops_1)):
        text = random.choice(texts_1)
        screen.blit(text, (i * font_ps, drops_1[i] * font_ps))
        drops_1[i] -= 1
        if drops_1[i] == -1 or random.random() > 0.93:
            drops_1[i] = int(height / font_ps)
    pygame.display.flip()
